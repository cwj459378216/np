...
export interface DnsTrendingData {
    timestamp: number;
    count: number;
}
...
@Component({
    ...
    templateUrl: './application-dns.component.html',
    styleUrls: ['./application-dns.component.css'],
    animations: [toggleAnimation]
})
export class ApplicationDnsComponent implements OnInit, OnDestroy {
    search = '';
    loading = false;
    chartLoading = false;
    currentPage = 1;
    ...
    loadTrendingData() {
        ...
        this.http.get<DnsTrendingData[]>(`${environment.apiUrl}/es/trending`, {
            params: {
    ...
    loadData() {
        ...
        this.http.get<DnsQueryResponse>(`${environment.apiUrl}/es/query`, {
            params: {
                startTime: startTime.toString(),
                endTime: endTime.toString(),
                index: 'dns-realtime',
            ...
            next: (response) => {
                this.rows = response.hits.map(hit => ({
                    ...
                    answers: this.truncateText(hit.answers)
                }));
    ...
    private printData(columns: string[], data: any[], filename: string) {
        ...
        if (printWindow) {
            printWindow.document.write('<html><head><title>DNS Queries</title></head><body>');
